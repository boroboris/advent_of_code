...
$passphrases_input = "vxjtwn vjnxtw sxibvv mmws wjvtxn icawnd rprh
...
function getPassphrasesFromText($passphrases_input)
{
    $passphrases = [];
    $temp = explode(PHP_EOL, $passphrases_input);
    foreach ($temp as $row) {
        $passphrases[] = preg_split('/\s+/', $row);
    }

    return $passphrases;
}

$passphrases = getPassphrasesFromText($passphrases_input);

/************************************************************/

$count_valid = 0;

foreach ($passphrases as $key => $passphrase) {
    $unique_array = array_unique($passphrase);

    if(count($unique_array) != count($passphrase)) {
...
echo "number of valids im p1: $count_valid<br>";
echo "<hr>";

/************************************************************/

$passphrases_test = "abcde fghij
abcde xyz ecdab
...
foreach ($passphrases as $key => $passphrase) {
    ...
    foreach ($passphrase as $word) {
        ...
        sort($letters);
        $sorted_words[] = implode('', $letters);
    }

    $unique_array = array_unique($sorted_words);
...
echo "number of valids in p2: $count_valid";
