...
function findGroups($stream) {

    $groups = []; $matches = [];

    while(preg_match_all("/({}),?|{(.+)}/", $stream, $matches) > 0 ) {
//        preg_match_all("/({[^\{\}]*},?)/", $stream, $matches);
        var_dump($matches);

        $score1 = array_count_values($matches[1]);
        $score2 = array_count_values($matches[2]);

        if(!empty($matches[2]) && count($matches[2]) <= 1) {
            $stream = $matches[2][0];
            $groups[] = 1;
        } else {
            $stream = implode('', $matches[2]);
            $groups[] = $score1['{}'] + $score2['{}'] + $score2["''"];
        }
//        $stream = preg_match("/{(.*)}/", "$1", $stream);
        var_dump($stream);
...
foreach ($test_cases as $key => $test) {
    ...
    foreach ($groups as $group_count) {
        $points += $group_count * $counter;

        $counter++;
    }

    if($points == $test_results[$key]) {
        Logger::outputLine('solution 1', $points);
    } else {
        Logger::outputLine("ERROR: should be $test_results[$key] instead of", $points);
    }
