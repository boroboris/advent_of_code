...
function initRegisters($commands, $memory_array)
{
    if (!key_exists($commands[0], $memory_array)) {
        $memory_array[$commands[0]] = 0;
    }

    if (!key_exists($commands[4], $memory_array)) {
        $memory_array[$commands[4]] = 0;
    }
...
foreach ($instructions as $instruction) {
    $commands = preg_split('/\s+/', $instruction);

    $memory_array = initRegisters($commands, $memory_array);

    $eval_result = true;
    if(array_key_exists($commands[4], $memory_array)) {
        $instruction_if = $memory_array[$commands[4]] . " " . $commands[5] . " " . $commands[6];

        eval("\$eval_result = " . $instruction_if . ";");

        if($eval_result) {
            ($commands[1] == 'inc') ? $memory_array[$commands[0]] += (int) $commands[2] : $memory_array[$commands[0]] -= (int) $commands[2];
        } else {
            continue;
        }
    }

    $max = max($memory_array);
    $max_all_time = ($max > $max_all_time) ? $max : $max_all_time;
}

$max = max($memory_array);

Logger::outputLine('solution p1', $max);
Logger::outputLine('solution p2', $max_all_time);
