...
class Program {
    ...
    public function __construct($name, $weight, $children)
    {
        $this->name = trim($name);
        $this->weight = $weight;
        ...
        foreach ($children as $child) {
            $this->children[] = trim($child);
        }
    }
}

function countNames($child, &$names_count)
{
    $child = trim($child);
    if (!key_exists($child, $names_count)) {
        $names_count[$child] = 0;
    }
...
function initPrograms($part1_string)
{
    ...
    $matches = [];
    preg_match_all("/([a-z]+)\s\((\d{1,})\)(\s->\s)?(.+)?/", $part1_string, $matches);
